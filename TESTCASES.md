...
### K. 自动评估（execution accuracy，57 条）

前置：`.env` 配好 LLM 与 DB；`python examples/eval_run.py` 在线跑（约 5 分钟，花真钱）。
比对逻辑由 `tests/test_eval.py` 离线单测守护。

| 用例组 | 覆盖 | R1 首轮 | R3 当前 |
|---|---|---|---|
| 指标值（准时率/收入/利用率/一次通过率/周期） | 17 条 | 11/13 | **17/17** |
| 分组（rows 比对） | 6 条 | 5/6 | 4/6 |
| 排名 TopN | 4 条 | 2/4 | 3/4 |
| 总量计数 | 8 条 | **0/8**（路由错进 RAG） | 7/8 |
| 多轮继承（同 session） | 9 条 | 9/9 | 9/9 |
| 澄清 / 文档RAG / 安全 / 边界 | 13 条 | 12/13* | 13/13 |
| **合计** | **57 条** | **75.4%** | **93.0%** |

*R1 时安全类按旧口径计。

已知失败（4 条，均有根因，详见 README §13.1）：LLM 自加过滤条件、id 列分组、保留字别名触发回退、
「合同总金额」双口径冲突（指标治理问题）。

| 检查项 | 预期 | 实际 | 结论 |
|---|---|---|---|
| K-01 | `python examples/eval_run.py --min-accuracy 0.9` 退出码 0 | | |
| K-02 | 失败用例在 `evals/last_report.json` 有 SQL 与明细 | | |
| K-03 | 修改语义层后重跑，通过率**不低于**基线 93%（防回归） | | |

...
- `那华南区呢？`（多轮）：继承了上一轮的**错误指标**（上一轮问收入，本轮答成了准时率）。
...
| 数据类别 | 目标库 | 内容 | 支撑的问数场景 |
|---|---|---|---|
| 业务板块经营 | PostgreSQL `business_segment_revenue` | 10 个季度 × 7 板块的营收/同比/毛利率 | 经营分析、同比、毛利率对比 |
| 实验室资源 | PostgreSQL `labs`（扩列） | 成立年份/设备台数/在职人数 | 实验室运营监控、资源统计 |
| 设备日志 | ES/OpenSearch `device_events` | 1222 条事件流水 | 告警计数、分组、TopN（PPL/DSL） |
| 检测报告文本 | ES/OpenSearch `inspection_reports` | 2000 份报告全文 | 全文检索、知识库 RAG 语料 |
...
```
# 最近季度 2026-06-30：可靠性 12852.0 / 集成电路 5887.44（同比 5.21%）/ 软件测评（毛利率 61.8%）
# 同比最高板块: 集成电路测试与分析 = 5.21%   2025 全年营收合计: 164880.4
```
